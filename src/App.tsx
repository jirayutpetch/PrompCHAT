...
import logoImage from './assets/images/promptchat-logo.png';
import avatarImage from './assets/images/promptchat-avatar.png';
import laptopImage from './assets/images/promptchat-laptop.png';
...
const embedScriptCode = (workspace?: RemoteWorkspace) => `<script\n  src="${window.location.origin}/widget.js"\n  data-promptchat-workspace="${workspace?.embed_key || 'promptchat-demo'}"\n  ${workspace ? `data-promptchat-domain="${workspace.domain}"\n  ` : ''}async\n></script>`;
...
function AuthScreen({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  return <motion.main className="auth-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: .45 }}>
    <section className="auth-visual">
      ...
      <div className="auth-brand"><img src={logo} alt="PromptCHAT" /><span>PromptCHAT</span></div>
      ...
      <div className="visual-art-wrap"><img className="visual-art" src={laptop} alt="PromptCHAT live chat mascot" /><div className="floating-note note-one"><span className="note-icon"><MessageCircle size={16} /></span><div><strong>ข้อความใหม่</strong><small>มีลูกค้ารอคำตอบ 2 คน</small></div></div><div className="floating-note note-two"><span className="note-icon blue"><Zap size={15} /></span><div><strong>ตอบกลับเร็วขึ้น</strong><small>ด้วยคำตอบลัดของทีม</small></div></div></div>
    ...
    <section className="auth-panel"><div className="auth-panel-inner">
      <div className="mobile-brand"><img src={logo} alt="PromptCHAT" /><span>PromptCHAT</span></div>
      ...
      <div className="auth-divider"><span>หรือ</span></div><button className="social-button" type="button" onClick={() => void googleLogin()}><span className="google-mark">G</span> ดำเนินการต่อด้วย Google</button><p className="auth-terms">การดำเนินการต่อถือว่าคุณยอมรับ <button className="text-link">เงื่อนไขการใช้งาน</button> และ <button className="text-link">นโยบายความเป็นส่วนตัว</button></p><div className="auth-security"><span className="security-lock">⌁</span> ข้อมูลของคุณถูกเข้ารหัสและปลอดภัย</div>
    </div></section>
  </motion.main>;
}
...
function Workspace({ onLogout, workspace, onSwitchWorkspace }: { onLogout: () => void; workspace?: RemoteWorkspace; onSwitchWorkspace?: () => void }) {
  ...
  return <motion.div className="app-shell" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: .35 }}>
    <header className="topbar"><div className="topbar-brand"><img src={logo} alt="PromptCHAT" /><span>Prompt<span>CHAT</span></span></div><div className="topbar-center"><button className="workspace-switcher" onClick={onSwitchWorkspace} disabled={!onSwitchWorkspace}><span className="workspace-mark">P</span><span>{workspace?.name || 'PromptCHAT workspace'}</span><ChevronDown size={15} /></button><div className="live-pill"><span className="status-dot" /> ออนไลน์</div></div><div className="topbar-actions"><button className="icon-button" aria-label="การแจ้งเตือน"><Bell size={19} />{conversations.some((item) => item.status === 'รอตอบ') && <i>{conversations.filter((item) => item.status === 'รอตอบ').length}</i>}</button><div className="topbar-avatar">PA</div><div className="user-summary"><strong>Prompt Admin</strong><small>เจ้าของ workspace</small></div><button className="icon-button" onClick={onLogout} aria-label="ออกจากระบบ"><LogOut size={17} /></button></div></header>
    <div className="app-body"><aside className="sidebar"><div className="sidebar-label">WORKSPACE</div><nav><SidebarItem icon={<LayoutDashboard size={18} />} label="ภาพรวม" active={view === 'overview'} onClick={() => setView('overview')} /><SidebarItem icon={<MessageCircle size={18} />} label="กล่องข้อความ" active={view === 'inbox'} badge={String(conversations.filter((item) => item.status === 'รอตอบ').length)} onClick={() => setView('inbox')} /><SidebarItem icon={<Users size={18} />} label="ผู้เยี่ยมชม" active={view === 'visitors'} onClick={() => setView('visitors')} /></nav><div className="sidebar-label sidebar-label-spaced">จัดการระบบ</div><nav><SidebarItem icon={<Code2 size={18} />} label="ติดตั้งบนเว็บไซต์" active={view === 'install'} onClick={() => setView('install')} /><SidebarItem icon={<PlugZap size={18} />} label="API & integrations" active={view === 'api'} onClick={() => setView('api')} /><SidebarItem icon={<Settings size={18} />} label="ตั้งค่า" active={view === 'settings'} onClick={() => setView('settings')} /></nav><div className="sidebar-spacer" /><div className="sidebar-help"><div className="help-icon"><Headphones size={18} /></div><div><strong>ต้องการความช่วยเหลือ?</strong><p>ทีมของเราพร้อมดูแลคุณ</p><button onClick={() => setToast('ทีมงานจะติดต่อกลับเร็ว ๆ นี้')}>คุยกับทีมงาน <span>→</span></button></div></div><div className="sidebar-footer"><div className="mini-avatar">PA</div><div><strong>PromptCHAT</strong><small>{workspace ? workspace.domain : 'Local preview'}</small></div><MoreHorizontal size={17} /></div></aside>
      <main className="workspace-main">{view === 'inbox' && <InboxView conversations={filteredConversations} active={activeConversation} filter={filter} query={query} setFilter={setFilter} setQuery={setQuery} onSelect={setActiveId} composer={composer} setComposer={setComposer} onSend={sendMessage} onSendFile={sendFile} onDone={markDone} onEmbed={() => setShowEmbed(true)} onPreview={() => setShowWidget(true)} onSimulate={workspace ? undefined : simulateVisitor} agentIcon={agentIcon} quickReplies={quickReplies} />}{view === 'overview' && <OverviewView conversations={conversations} onInbox={() => setView('inbox')} />}{view === 'visitors' && <VisitorsView conversations={conversations} />}{view === 'install' && <InstallView onCopy={copyEmbed} code={embedScriptCode(workspace)} workspace={workspace} />}{view === 'api' && <ApiView code={embedScriptCode(workspace)} />}{view === 'settings' && <SettingsView botEnabled={botEnabled} setBotEnabled={setBotEnabled} agentIcon={agentIcon} setAgentIcon={setAgentIcon} onSave={() => setToast('บันทึกการตั้งค่าแล้ว')} />}</main>
    </div>
...
function InstallView({ onCopy, code, workspace }: { onCopy: () => void; code: string; workspace?: RemoteWorkspace }) { const [launcher, setLauncher] = useState<'default' | 'custom'>('default'); const installCode = launcher === 'custom' ? code.replace('  async', '  data-promptchat-launcher="custom"\n  async') : code; const copyInstall = () => launcher === 'custom' ? void navigator.clipboard?.writeText(installCode) : onCopy(); return <div className="content-page narrow-content"><div className="page-heading"><div><div className="breadcrumb">จัดการระบบ / ติดตั้งบนเว็บไซต์</div><h1>นำ PromptCHAT ไปไว้บนเว็บคุณ</h1><p>ติดตั้งวิดเจ็ตด้วยโค้ดสั้น ๆ เพียงครั้งเดียว แล้วเริ่มรับข้อความจากลูกค้าได้ทันที</p></div></div><div className="install-grid"><div className="install-card panel-card"><div className="install-step"><span>01</span><div><h3>คัดลอกโค้ดติดตั้ง</h3><p>วางโค้ดนี้ก่อนปิดแท็ก <code>&lt;/body&gt;</code> ในเว็บไซต์ของคุณ</p></div></div><div className="launcher-choice"><span>ปุ่มเปิดแชต</span><label><input type="radio" name="launcher" checked={launcher === 'default'} onChange={() => setLauncher('default')} /> ใช้ปุ่ม PromptCHAT</label><label><input type="radio" name="launcher" checked={launcher === 'custom'} onChange={() => setLauncher('custom')} /> ใช้ปุ่มของเว็บไซต์ฉัน</label></div><div className="code-block"><button onClick={copyInstall}><Clipboard size={15} /> คัดลอก</button><pre>{installCode}</pre></div>{launcher === 'custom' && <div className="custom-launcher-guide"><p>ระบบจะซ่อนปุ่ม PromptCHAT แต่ยังแสดงหน้าต่างแชตเมื่อปุ่มของเว็บเรียกคำสั่งนี้:</p><pre>{'<button onclick="window.PromptChatWidget.open()">แชตกับเรา</button>'}</pre><small>มีคำสั่ง <code>open()</code>, <code>close()</code> และ <code>toggle()</code> ให้ใช้งาน</small></div>}<div className="install-tip"><Sparkles size={17} /><span>หลังติดตั้งแล้ว ลองเปิดเว็บของคุณในอีกแท็บหนึ่งเพื่อทดสอบการแชต</span></div>{workspace && <InstallVerification workspace={workspace} />}</div><div className="install-card side-install panel-card"><div className="install-step"><span>02</span><div><h3>ปรับให้เข้ากับแบรนด์</h3><p>เปลี่ยนสี ข้อความต้อนรับ และตำแหน่งวิดเจ็ตได้จากหน้า “ตั้งค่า”</p></div></div><div className="theme-preview"><div className="theme-preview-top"><img src={avatar} alt="PromptCHAT" /><div><strong>PromptCHAT</strong><small>ทีมงานออนไลน์</small></div><X size={14} /></div><div className="theme-preview-msg">สวัสดีค่ะ มีอะไรให้เราช่วยไหมคะ?</div><div className="theme-preview-input">พิมพ์ข้อความ...</div></div><button className="outline-button full" onClick={onCopy}><Code2 size={16} /> เปิดหน้าตั้งค่า</button></div></div></div>; }
...
function EmbeddedWidget() {
  ...
  const [widgetName, setWidgetName] = useState('PromptCHAT');
